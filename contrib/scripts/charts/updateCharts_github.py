...
class GithubClient:
    ...
    def checkResponse(self, repo, method, param=""):
        url = self.url + "/repos/PeonyTeam/%s/%s" % (repo, method)
        if param != "":
            url += "/%s" % param
    ...
    def getContributors(self, repo='peony'):
        return self.checkResponse(repo, "stats/contributors")

    def getPulls(self, sorting='created', repo='peony'):
        pulls = []
        try:
            i = 1
            while True:
    ...
    def getWeeklyCommits(self, repo='peony'):
...
class GeckoClient:
    ...
    def __init__(self):
        self.url = "https://api.coingecko.com/api/v3/coins/peony"
