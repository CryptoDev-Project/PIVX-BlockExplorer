...
// PnYParser handle
type PnYParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewPnYParser returns new PnYParser instance
func NewPnYParser(params *chaincfg.Params, c *btc.Configuration) *PnYParser {
	p := &PnYParser{
...
// GetChainParams contains network parameters for the main PnY network
...
func (p *PnYParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
...
func (p *PnYParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *PnYParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *PnYParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *PnYParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *PnYParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *PnYParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *PnYParser) IsAddrDescIndexable(addrDesc bchain.AddressDescriptor) bool {
...
func (p *PnYParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
...
func (p *PnYParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
	if len(tx.Vin) > input {
		scriptHex := tx.Vin[input].ScriptSig.Hex
		if scriptHex != "" {
			script, _ := hex.DecodeString(scriptHex)
...
func (p *PnYParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
...
func (p *PnYParser) P2CSScriptToAddress(script []byte) ([]string, bool, error) {
	if len(script) < 50 {
